...
bool collide_snake_body(vec2 point) {
    for (int i = 0; i < score; i++) if (collide(point, segments[i])) return true;
    return false;
}
...
void draw_border(int y, int x, int width, int height) {
    ...
    for (int i = 1; i < width * 2 + 1; i++) mvaddch(y, x + i, ACS_HLINE);
    // vertical lines
    ...
    for (int i = 1; i < width * 2 + 1; i++) mvaddch(y + height + 1, x + i, ACS_HLINE);
}
...
void update() {
    ...
    for (int i = score; i > 0; i--) segments[i] = segments[i - 1];
    segments[0] = head;
